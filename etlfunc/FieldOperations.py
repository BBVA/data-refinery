...
from etlfunc.tuple.TupleOperations import compose
...
def type_enforcer(enforcer_function: Callable[[T], U]) -> \
    ...
    def _app(x: T, e: str=None) -> Tuple[Optional[U], Optional[str]]:
        ...
        try:
            ...
            return None, f"can't cast {x} to enforced type {e}"
    ...
    def _app(x: float, e=None) -> Tuple[Optional[int], Optional[str]]:
        ...
        for (lower, upper, index) in intervals:
            if lower is None and x <= upper:
                ...
                return index, None
        return None, f"bucket not found for {x}"
...
def linear_category(categories: List[T]) -> \
    ...
    def _app(x: T, e: str=None) -> Tuple[Optional[int], Optional[str]]:
        ...
        if x not in category_map:
            return None, f"value {x} not found on categories"
...
def add_column_prefix(prefix):
    def _app(x, e=None):
        if x is not None:
            return {f"{prefix}_{k}": v for (k, v) in x.items()}, None
        return None, e
    return _app
...
def date_parser(date_formats: list):
    def _app(x: str, e=None):
        if x is None:
            return None, f"Date can't be None: {x}"
        ...
        for current_format in date_formats:
            try:
                ...
                continue
        return None, f"Can not parse date {x}"
...
def time_parser(formats: list):
    def _app(x: str, e=None):
        if x is None:
            return None, f"Time can't be None: {x}"
        if formats is None:
        ...
        for current_format in formats:
            try:
                ...
                continue
        return None, f"Can not parse time {x}"
...
def match_dict(dictionary):
    def _app(x, e=None):
        ...
        return None, f"{x} not found on dictionary"
    return _app
