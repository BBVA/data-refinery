...
from datarefinery.tuple.TupleOperations import keep, substitution, fusion, append, filter_tuple, wrap, change, \
...
def test_change():
    operation = Tr(substitution(["a"], etl_func=wrap(lambda x: x.upper())))\
        .then(change(fields=["a"], etl_func=wrap(lambda x: "{}!!!".format(x))))\
        .apply()
...
def test_fusion_append():
    operation = Tr(fusion_append(
        ...
        etl_func=wrap(lambda x: {"greet_person": " ".join(x), "greet_world": "{} world".format(x[0])}))
    )\
        .apply()
